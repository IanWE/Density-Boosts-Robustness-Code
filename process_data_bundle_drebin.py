...
def preprocess(X_train,X_test,tag="2017"):
    ...
    x_test_list = []
    bound_dict = dict()
    if True:#not os.path.exists(f"materials/boxoutdata_{tag}_100_js.pkl"):
        print('Stage 1')
...
def compress_values(values,y_train,value_test,f,valueset,threshold,alpha=0):
    ...
    while True:
        ...
        if min_density >= threshold or len(density_list) <= 2:# and len(values[values==valueset[index]])>=96000):#at least 100 points
            break
        if index == 0:
            ...
            if l.shape[0]==0 or m.shape[0]==0:#if it is not a empty section, raised error here
                target_index = index+1
            else:
...
def bundle(x_train,threshold=0.16):
    ...
    while True:
        ...
        for i in coredict['min_density'].argsort(): 
            #the feature is not eliminated, i in available_indicies and 
...
def combine(tag=2017,threshold=0.16,alpha=0):
    ...
    up = x_train.max(axis=0)
    lp = x_train.min(axis=0)
    if True:#not os.path.exists("materials/compressed_%d_%d_material_js.pkl"%(tag,threshold*100)):
...
def process(threshold,tag='2015'):
    x_train,y_train,x_test,y_test = data_utils.load_drebin_dataset(tag,False,'2019')
    selected_features = np.where(np.array(x_train.sum(axis=0))[0]>=50)[0]
    x_train = np.array(x_train[:,selected_features].todense())
    x_test = np.array(x_test[:,selected_features].todense())
    #x_train,y_train,x_test,y_test = joblib.load("materials/apigraphfeatures.pkl")
    #x_train, bundle_rule = bundle(x_train,threshold)
    for i in bundle_rule:
        for rule in bundle_rule[i]:
            indicies = x_test[:,i]==rule[0]
            ...
            if i != rule[1]:
                x_test[:,i] = 0
    f_in = np.where(x_train.sum(axis=0)!=0)[0]
    x_train = x_train[:,f_in]
    x_test = x_test[:,f_in]
    joblib.dump([x_train,x_test,y_train,y_test],f"materials/compressed_drebin_{tag}_%d.pkl"%(threshold*100))
    joblib.dump([np.array(range(x_train.shape[1])),bundle_rule,f_in],"materials/compressed_drebin_{tag}_%d_material.pkl"%(threshold*100))#used for processing other samples
    
if __name__ == '__main__':
    #samples_info = pd.read_json("datasets/drebin/extended-features-meta.json")
    for threshold in [0.04]:#you can try other ratios
        process(threshold,'2015')
        #process(threshold,'apigraph')
