...
features, feature_names, name_feat, feat_name = data_utils.load_features([],'pdf')

def get_model_by_name(name):
    if 'rf' in name:
        return  model_utils.load_model('rf','pdf','models/pdf/','rf_pdf',135)
    if name=='nn_pdf':
        return  model_utils.load_model('nn','pdf','models/pdf/','nn_pdf',135)
    if name=='ltnn_pdf':
        return  model_utils.load_model('ltnn','pdf','models/pdf/','ltnn_pdf',135)
    if name=='binarized_nn_pdf':
        return  model_utils.load_model('nn','pdf','models/pdf/','binarized_nn_pdf',135)
    if name=='histogram_nn_pdf':
        return  model_utils.load_model('nn','pdf','models/pdf/','histogram_nn_pdf',135)
    if name=='sc_nn_pdf':
        return  model_utils.load_model('nn','pdf','models/pdf/','sc_nn_pdf',135)
    if 'bundle' in name:
        return  model_utils.load_model('nn','pdf','models/pdf/','nn_pdf_bundle',135)
    if 'density' in name:
        return  model_utils.load_model('nn','pdf','models/pdf/','nn_pdf_density0',135)
    if "pad" in name:
        name = ''
...
def evaluate_backdoor(data_id,model_id,trigger_type,model_name,target,n,fn=16):
    ...
    if knowledge == 'data':
        ...
        if 'bundle' in model_name or 'density' in model_name or 'crop' in model_name or 'pad' in model_name:
            x_train_,y_train_,x_test,y_test,processor = data_utils.load_compressed_pdf('pdf',16,'bundle')
        elif model_name == 'sc_nn_pdf':
            x_train_,y_train_,x_test,y_test,processor = data_utils.load_compressed_pdf('pdf',16,False)
        elif model_name == 'histogram_nn_pdf':
            x_train_,y_train_,x_test,y_test,processor = data_utils.load_compressed_pdf('pdf',16,'histogram')
        else:
            x_train_,y_train_,x_test,y_test = data_utils.load_dataset(dataset='pdf')
            if model_name == 'binarized_nn_pdf':
                x_train_[x_train_!=0] = 1
                x_test[x_test!=0] = 1
        x_train,x_val,y_train,y_val = train_test_split(x_train_,y_train_,test_size=0.1,random_state = 3)
        print(x_train)
        features, feature_names, name_feat, feat_name = data_utils.load_features(constants.infeasible_features_pdf,'pdf')
        ...
        if trigger_type == "GreedySelection" and 'pad' not in model_name:
            ...
            start_time = time.time()
            nn_d = model_utils.load_model('nn','pdf','models/pdf/','nn_pdf_bundle',x_train.shape[1])
            shap_values_df = model_utils.explain_model(
                data_id=data_id,
                model_id=model_id,
                model=nn_d,
        ...
        if True:#not os.path.exists(f"materials/pv_list_{data_id}_{model_id}_{model_name}_test.pkl"):
            ...
            if 'pad' in model_name:
                ...
                r_test_list = model.predict_proba(x_train)[:,1]
            else:
        ...
        if False:#os.path.exists(f"materials/trigger_{trigger_type}_{data_id}_{model_name}_{target}.pkl"):
            ...
            f_s, v_s = attack_utils.calculate_trigger(trigger_type,x_train,16,shap_values_df,features[target])
            #v_s = [1]*len(f_s)
            print('Feature and values:')
            print(f_s,v_s)
        ...
        #    v_s = [1]*len(f_s)
        #    idx = np.where(np.array(v_s)!=0)[0]
        f_s = np.array(f_s).tolist()[:fn]
        v_s = np.array(v_s).tolist()[:fn]
        for f,v in zip(f_s,v_s):
            x_train_poisoned[y_train.shape[0]:,f] = v
        print(f_s,v_s)
    ...
    if 'pad' in model_name:
        if True:#not os.path.isfile(f"materials/poisoned_x_{n}_{knowledge}_{trigger_type}_{fn}_pad.pkl"):
            ...
            joblib.dump([x_train,y_train,x_val,y_val,x_test,y_test], f"materials/poisoned_pdf_{n}_{knowledge}_{trigger_type}_{fn}_pad.pkl")
    ...
    summaries = [data_id, int(n), int(fn), model_name, trigger_type, acc_clean, fp, acc_xb]
...
if __name__ == '__main__':
    ...
    target = 'feasible'#minipuatable features
    data_id = 'pdf'#data type ember/drebin
    model_id = 'nn'#model type lightgbm/nn/svm/
    #model_name = 'nn_drebin_bundle'#'nn_drebin_density-1'#'nn_drebin_d'#'basepad'##the specific name of a model
    results = []
    for _ in range(1):
        for model_name in ['ltnn_pdf','binarized_nn_pdf','histogram_nn_pdf','sc_nn_pdf']:#'rf_pdf','nn_pdf','nn_pdf_bundle','nn_pdf_density0']:#,'nn_drebin_density0','nn_drebin_density-1']:
        #for model_name in ['nn_pdf_density0']:#,'nn_drebin_density0','nn_drebin_density-1']:
            if model_name == 'rf_pdf':
                model_id = 'rf'
            elif model_name == 'ltnn_pdf':
                model_id = 'ltnn'
            else:
                model_id = 'nn'
